...
public class IndexShard extends AbstractIndexShardComponent implements IndicesClusterStateService.Shard {
    ...
    //////new SearchSlowLog(indexSettings) new ShardSearchStats()
    private final SearchOperationListener searchOperationListener;

    //分片
    protected volatile ShardRouting shardRouting;
    //IndexShardState.CREATED IndexShardState.RECOVERING IndexShardState.POST_RECOVERY
    ...
    //////new IndexingSlowLog(indexSettings) IndexingMemoryController new InternalIndexingStats()
    ...
    //primaryReplicaSyncer = primaryReplicaSyncer = primaryReplicaSyncer::resync
    @Override
    public void updateShardState(final ShardRouting newRouting,
        ...
        synchronized (mutex) {
            ...
            //索引已经恢复，分片状态是 STARTED 或 RELOCATING
    ...
    public static Engine.Index prepareIndex(DocumentMapperForType docMapper, Version indexCreatedVersion, SourceToParse source, long seqNo,
        ...
        //解析传入的json数据，如果有新字段映射生成会存在 ParsedDocument.dynamicMappingsUpdate.root.mappers  中
        //ParsedDocument.documents 多个文档集合
        ParsedDocument doc = docMapper.getDocumentMapper().parse(source);
        if (docMapper.getMapping() != null) {
            //把 docMapper 的字段 mapping 和  doc 的字段 Mapping 一起合并
    ...
    //recoveryListener = new RecoveryListener(shardRouting)
    ...
    //type = type 名字
    //根据 type名字 获取对应 mapping
    private DocumentMapperForType docMapper(String type) {
        return mapperService.documentMapperWithAutoCreate(type);
    }

    //EngineConfig.OpenMode.CREATE_INDEX_AND_TRANSLOG OPEN_INDEX_CREATE_TRANSLOG
    //创建
